...
exports.format = function(req,cb){
    ...
    if(typeof req.display === "function"){
      ...
      // Adding reject here for single purpose of adflute: optimizer 
      //whose all fields are already defined in display but whatever 
      //fields are requested in request those only will be returned back.
      logger.info(req.params.measure);
	logger.info(req.params.dayWise);
      display = _.reject(display, function(obj){ return !(((req.params.dayWise=='true')? obj.key=="daydate" : false) || (obj.key.indexOf('_id')==-1 && req.params.measure.indexOf(obj.key)!=-1) || (obj.key.indexOf('_id')!=-1 && (req.params.fields.indexOf(obj.key)!=-1 && (obj.name=="app_name"? req.params.isAdminReqAppName=='true' : true)) )) });
	logger.info(display);
    }
    ...
    try{
      ...
      if(!dp.format){ // no format case
        ...
        process.nextTick(function(){
            ...
            dp.format(row,req,val,dp.key,cacheMap,function(output){
