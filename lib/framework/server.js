          ...
          if(debugFlag === "validate") { 
            ...
            if(validReq.model.raw){ //added to provie interface to build raw query execution and formatting logic
              ...
              query.build(validReq,function(err,queryReq){ // builds queries (main,count, joins)
				...
				logger.info(queryReq.display);
                                var display = _.reject(queryReq.display, function(obj){ return !(obj.key=="daydate" || (obj.key.indexOf('_id')==-1 && params.measure.indexOf(obj.key)!=-1) || (obj.key.indexOf('_id')!=-1 && (params.fields.indexOf(obj.key)!=-1 && (obj.name=="app_name"? params.isAdminReqAppName=='true' : true)) )) });
                                display = _.indexBy(display, 'name')
                                logger.info("csv generator place. .......................");
                                logger.info(display);
                                logger.info(output.data);
                                json2csv({ data: output.data, fields: Object.keys(display) }, function(err, csv) {
